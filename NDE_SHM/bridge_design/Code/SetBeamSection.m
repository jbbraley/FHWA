...
switch Parameters.structureType
    case {'Steel', 'steel'}
        ...
        for ii = 1:length(Section)
            ...
            if isfield(Parameters.Beam.(Section{ii}).CoverPlate,'Length') && Parameters.Beam.(Section{ii}).CoverPlate.Length > 0
    ...
    case 'Prestressed'
        ind = find(strcmp({Parameters.St7Prop(:).propName},'Girder Concrete'));
        SetPSSection(uID,Parameters.Beam.Int, Parameters.St7Prop(ind).St7PropNum); % Prestressed only uses Int field
end  
end     
...
function SetPSSection(uID, Beam, PropNum)

oldcd = pwd;
ffname = mfilename('fullpath');
[sxn_dir, ~, ~] = fileparts(ffname);
cd([sxn_dir '\Prestressed\Sections']);
