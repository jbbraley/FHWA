...
% Last Modified by GUIDE v2.5 09-Feb-2015 14:03:54
...
function guiImportTestData_gui_CloseRequestFcn(hObject, eventdata, handles)
delete(hObject);
...
% Get file location
[FileName, FilePath] = uiputfile({'*.mat'}, 'Save Results File As...');
...
testData.x =  testData.x + Xoff;
testData.y =  testData.y + Yoff;

% Determined edge boundaries for test data using nodal data and add to test
% data
testData.xB = meshData.xB;
testData.yB = meshData.yB;
...
setappdata(0, 'testFileName', FileName);
setappdata(0, 'testPathName', FilePath);

guiImportTestData_gui_CloseRequestFcn(handles.guiImportTestData_gui, eventdata, handles)
...
% impacts 
testCoord = unique(testCoord, 'rows');
...
testData.U = ones(length(testCoord), length(results.freq))*NaN;
...
for i = 1:length(results.freq)
    ...
    [~, LOCB] = ismember([results.x{i}, results.y{i}], testCoord, 'rows');  
    testData.U(LOCB,i) = results.U{i};
end

% convert from feet to inches
testCoord = testCoord*12;
testData.x = testCoord(:,1);
testData.y = testCoord(:,2);
...
%% Graphical Functions -----------------------------------------------------
function printGrid(handles, meshData)
% Reprint the Grid
...
hold on
guiH.grid = scatter(handles.axesGrid, meshData.x, meshData.y,'b.');
scatter(meshData.xB, meshData.yB, 'r.');
hold off
setappdata(handles.guiImportTestData_gui,'guiH',guiH);
...
hold on
guiH.test = scatter(handles.axesGrid, testData.x, testData.y,'go','fill');
hold off
...
coord_prime = [meshData.x, meshData.y];
...
switch OffsetData.CurrentAxis
    case 'X'
        meshData.axisX = dist;
        OffsetData.X = coord_prime(meshData.axisX,1);
    ...
    case 'Y'
        meshData.axisY = dist;
        OffsetData.Y = coord_prime(meshData.axisY,2);
...
if isfield(OffsetData,'X')
    ...
    testData.x =  testData.x + OffsetData.X;
end
if isfield(OffsetData,'Y')
    ...
    testData.y =  testData.y + OffsetData.Y;
end
