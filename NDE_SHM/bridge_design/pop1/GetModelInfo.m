...
ModelSpace(ii).Deck_t = Parameters.Deck.t;
ModelSpace(ii).Deck_fc = Parameters.Deck.fc;
...
%     ModelSpace(ii).NumStrands = Parameters.Beam.PSSteel.NumStrands;
    ...
    if strcmp(Parameters.Beam.Type, 'Rolled')
        ...
        ModelSpace(ii).SectionName = 'Plate';
        
    end
    ModelSpace(ii).Mn_pos = Parameters.Beam.Int.Mn_pos;
    ModelSpace(ii).bft = Parameters.Beam.Int.bf; %width top flange
    ModelSpace(ii).tw = Parameters.Beam.Int.tw; %thickness web
    ModelSpace(ii).tft = Parameters.Beam.Int.tf; %thickness top flange
    ModelSpace(ii).tfb = Parameters.Beam.Int.tf; % thickness of bottom blange
    ModelSpace(ii).bfb = Parameters.Beam.Int.bf; % width of bottom flange
