...
if Parameters.Dia.Stagger==0 % Parallel with skew
    LDiv = SpanLength./(Parameters.NumDia+1);
elseif Parameters.Dia.Stagger==1 % Normal to girders
    LDiv = TotalLength./(Parameters.NumDia+1);
else
...
if Parameters.Overhang > 0
    ...
    OMesh = 1;
    OMeshSize = Parameters.Beam.bf/2;
end
...
for h = 1:Parameters.Spans % Begin loop for span node creation
    ...
    for i=1:WNodesTotal
        ...
        if rem((i-1),WMesh)==0
            ...
            Node(h).x(n)= nearxcoord;
            Node(h).y(n)= nearycoord;
            Node(h).z(n)=-BeamDepth/2-Parameters.Deck.t/2;
            Node(h).Type(n,:) = 'GMid';
            Node(h).Location(n,:) = 'NAbut';
            ...
            Node(h).x(n)= nearxcoord;
            Node(h).y(n)= nearycoord;
            Node(h).z(n)=-Parameters.Deck.t/2;
            Node(h).Type(n,:) = 'GTop';
            Node(h).Location(n,:) = 'NAbut';
            Node(h).ID(1,i,2)=n;
            ...
            Node(h).x(n)= nearxcoord;
            Node(h).y(n)= nearycoord;
            Node(h).z(n)=-BeamDepth-Parameters.Deck.t/2;
            Node(h).Type(n,:) = 'GBot';
            Node(h).Location(n,:) = 'NAbut';
            Node(h).ID(1,i,4)=n;
            n=n+1;
...
for h=1:Parameters.Spans
    ...
    Connection=0;
    EltNum = 0; % reset element number counter
    for i=1+OMesh:WMesh:(Parameters.NumGirder-1)*WMesh+1+OMesh
        ...
        while k<size(NodeID,1) && NodeID(k+1,i,3)~=0
            ...
            Node(h).ElementCon(NodeID(k,i,3),1) = EltNum;
            Node(h).ElementCon(NodeID(k+1,i,3),2) = EltNum;
%             try
%             if Node(h).ElementCon(NodeID(k,i,3)-NodeStart(h)) == 0
%                 Node(h).ElementCon(NodeID(k,i,3)-NodeStart(h),1) = EltNum;
%             else    
%                 Node(h).ElementCon(NodeID(k,i,3)-NodeStart(h),2) = EltNum;
%             end
% 
%             if Node(h).ElementCon(NodeID(k+1,i,3)-NodeStart(h)) == 0
%                 Node(h).ElementCon(NodeID(k+1,i,3)-NodeStart(h),1) = EltNum;
%             else    
%                 Node(h).ElementCon(NodeID(k+1,i,3)-NodeStart(h),2)= EltNum;
%             end
%             catch
%                g 
%             end
            %% Assign Cover Plate properties to negative moment regions
            if Parameters.Beam.CoverPlate.Length(1) ~= 0 && (h==1 && k>kEnd-CoverPl(h)) || (h==length(Parameters.Length) && k<CoverPl(h)+kStart) ||...
                    (h~=length(Parameters.Length) && h~=1 && (k<CoverPl(h)+kStart-1 || k>kEnd-CoverPl(h)))
                PropNum = 8;
                % iErr = calllib('St7API', 'St7SetBeamOffset2', uID, EltNum, [0 (Parameters.Beam.yB-Parameters.Beam.d/2)]);
                % HandleError(iErr);
            else
                PropNum = 5;
            end
            
            iErr = calllib('St7API', 'St7SetElementConnection', uID, tyBEAM, EltNum, PropNum, Connection);
            HandleError(iErr);
            iErr = calllib('St7API', 'St7SetEntityGroup', uID, tyBEAM, EltNum, 1);
            HandleError(iErr);
            k=k+1;
        end
    end
    ...
    if Parameters.Barrier.Width>0
        % Right Barrier
        PropNum=Parameters.Barrier.St7PropNum(1);
        Connection=0;
        ...
        % Left Barrier
        PropNum=Parameters.Barrier.St7PropNum(2);
        Connection=0;
