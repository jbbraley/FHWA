%% DESIGNS SECTION  WITH A COVER PLATE FOR NEGATIVE MOMENT REGION
% All parameters for negative moment region are saved in
% Paramters.Beam.CoverPlate

function [Parameters, exitflag] = PlateGirderDesignLRFD_CP(Parameters, CShapes, LShapes)
...
while designFalse == 1 && count < 11
    ...
    % x0 = [flange width  flange thickness  web thickness cover plate flange thickness]
    ub = [Parameters.GirderSpacing/3 Parameters.GirderSpacing/20 Parameters.GirderSpacing/20 Parameters.GirderSpacing/20];
%     ub = [18 1.5 1.5 3];
    lb = [6 .5 1/4 .5];
...
%     x0(4) = lb(4)+1;
    ...
    if all(c<=0)
        ...
        Parameters.Beam.CoverPlate = Parameters_cp.Beam;
        Parameters.Beam.CoverPlate.Length = Parameters.Design.CoverPlate.Ratio*Parameters.Length;
        Parameters.Beam.CoverPlate.Section = Parameters.Beam.Section;
        Parameters.Beam.CoverPlate.Section(4) = Parameters_cp.Beam.tf; % Beam w/ cover plate section geometry
        Parameters.Beam.CoverPlate.Section(5) = Parameters_cp.Beam.tf;
...
A = (1-Parameters.Design.CoverPlate.Ratio)*(2*x(1)*x(2) + x(3)*(Parameters.Beam.d - 2*x(2))) + Parameters.Design.CoverPlate.Ratio*(2*x(1)*x(4) + x(3)*(Parameters.Beam.d - 2*x(2)));
end
...
% Assign Parameters.Beam properties
    ...
    Parameters_cp = Parameters; %Creates temporary file for cover plate region
    Parameters_cp.Beam.tf = x(4); % Thickness of flange with cover plate
    Parameters_cp.Beam.t = Parameters_cp.Beam.tf - Parameters.Beam.tf;
    Parameters_cp.Beam.d = Parameters.Beam.d + 2*Parameters_cp.Beam.t; %New total depth of beam with coverplate
    Parameters_cp.Beam.Dt = Parameters_cp.Beam.d+Parameters.Deck.t; %Caclulates total section depth (with coverplate) for ductility requirement
    Parameters_cp.Beam.ind = Parameters_cp.Beam.d-2*Parameters_cp.Beam.tf;
    Parameters_cp.Beam.Ix = 2*(Parameters_cp.Beam.bf*Parameters_cp.Beam.tf^3/12 + Parameters_cp.Beam.bf*Parameters_cp.Beam.tf*...
        (Parameters_cp.Beam.tf/2+Parameters_cp.Beam.ind/2)^2)+ Parameters_cp.Beam.tw*Parameters_cp.Beam.ind^3/12;
    Parameters_cp.Beam.Iy = 2*Parameters_cp.Beam.tf*Parameters_cp.Beam.bf^3/12+(Parameters_cp.Beam.d-2*Parameters_cp.Beam.tf)*Parameters_cp.Beam.tw^3/12; %Moment of inertia of the entire steel section
    Parameters_cp.Beam.A = 2*Parameters_cp.Beam.bf*Parameters_cp.Beam.tf + Parameters_cp.Beam.tw*Parameters_cp.Beam.ind;
...
%%(2) Diapragm, Section Forces, Flexural Resistance, Ductility
    ...
    Parameters_cp = GetSectionForces(Parameters_cp); %cover plate section
    %Get LRFD Beam capacity
    Parameters = GetLRFDResistance(Parameters);
    Parameters_cp = GetLRFDResistance(Parameters_cp);%Cover plate section
    % 6.10.7.3 Ductility
    c(3) = Parameters.Beam.Dpst/Parameters.Beam.Dt - 0.42; 
    c(4) = Parameters_cp.Beam.Dpst/Parameters_cp.Beam.Dt - 0.42; % Check ductility for section with cover plate
    
%%(3) Web Thickness Criteria (6.7.3)
    c(5) = 0.3125 - Parameters.Beam.tw; % Web thickness remains the same regardless of cover plate
    
%%(4) Cross-Section Proportion Limits
    % Web Proportions 6.10.2.1 (for section without cover plate)
    c(6) = Parameters.Beam.ind/Parameters.Beam.tw - 150; % For webs without longitudinal stiffeners
    % Web Proportions 6.10.2.1 (for section with cover plate)
    c(7) = Parameters_cp.Beam.ind/Parameters.Beam.tw - 150; % For webs without longitudinal stiffeners
    % Flange Proportions 6.10.2.2 (for section without cover plate)
    c(8) = Parameters.Beam.bf/(2*Parameters.Beam.tf) - 12;
    c(9) = Parameters.Beam.d/6 - Parameters.Beam.bf;
    c(10) = 1.1*Parameters.Beam.tw - Parameters.Beam.tf;
    c(11) = 0.1 - Parameters.Beam.Iyc/Parameters.Beam.Iyt;  
    c(12) = Parameters.Beam.Iyc/Parameters.Beam.Iyt-10; 
    % Flange Proportions 6.10.2.2 (for section with cover plate)
    c(13) = Parameters_cp.Beam.bf/(2*Parameters_cp.Beam.tf) - 12;
    c(14) = 1.1*Parameters_cp.Beam.tw - Parameters_cp.Beam.tf;
    c(15) = 0.1 - Parameters_cp.Beam.Iyc/Parameters_cp.Beam.Iyt;  
    c(16) = Parameters_cp.Beam.Iyc/Parameters_cp.Beam.Iyt-10;
    c(17) = Parameters_cp.Beam.tf - 3*Parameters.Beam.tf;
    c(18) = Parameters.Beam.tf - Parameters_cp.Beam.tf;

%%(5) Constructibility Proportion Limits (6.10.3)

%%(6a) Service Limit State II for Positive Flexure (6.10.4)
    c(19) = Parameters.Beam.fbc_pos(2,:)-0.95*Parameters.Beam.Fy; % Top flange (in compression for positive flexure)
    c(20) = Parameters.Beam.fbt_pos(2,:)-0.95*Parameters.Beam.Fy; % Bottom Flange (in Tension for positive flexure)
    
%%(7a) Strength Limit State I for Positive Flexure (6.10.6)
    % Flexure Criteria
    c(21) = 2*Parameters.Beam.Dcp/Parameters.Beam.tw-3.76*sqrt(Parameters.Beam.E/Parameters.Beam.Fy); % 6.10.6.2.2
    c(22) = max(Parameters.LRFD.M_pos)-Parameters.Beam.Mn_pos; % 6.10.7.1 for compact sections
    %Shear Criteria
    c(23) = max(max(Parameters.LRFD.V))-Parameters.Beam.Vn;

%%Negative Moment Region (uses section properties for section with cover
%%plate)
...
%%(6b) Service Limit State II for Negative Flexure (6.10.4) 
    ...
    %c(22) = Parameters_cp.Beam.fbc_neg(2,:)-0.95*Parameters_cp.Beam.Fy; %Bottom flange
    %c(23) = Parameters_cp.Beam.fbt_neg(2,:)-0.95*Parameters_cp.Beam.Fy; %Top flange 
    c(24) = Parameters_cp.Beam.fbc_neg(2,:)-Parameters_cp.Beam.Fcrw;
...
%%(7b) Strength Limit State I for Negative Flexure (6.10.8)
    ...
    c(25) = Parameters_cp.Beam.fbc_neg(1,:)-Parameters_cp.Beam.Fn_neg; 
    ...
    % moment region
    c(26) = Parameters_cp.Beam.fbt_neg(1,:)-Parameters_cp.Beam.Fy;
...
%%(7a) Strength Limit State I for Positive Flexure (6.10.6)
    % Flexure Criteria
    c(19) = 3.76*sqrt(Parameters.Beam.E/Parameters.Beam.Fy)-2*Parameters.Beam.Dcp/Parameters.Beam.tw; % 6.10.6.2.2
    c(20) = max(Parameters.Beam.fbc_pos(1,:))-Parameters.Beam.Fn_pos; %Compression flange 6.10.7.2.1
    c(21) = max(Parameters.Beam.fbt_pos(1,:))-Parameters.Beam.Fn_pos; %Tension flange 
    %Shear Criteria
    c(22) = max(max(Parameters.LRFD.V))-Parameters.Beam.Vn;
...
%%(6b) Service Limit State II for Negative Flexure (6.10.4) 
    ...
    c(23) = Parameters_cp.Beam.fbc_neg(2,:)-Parameters_cp.Beam.Fcrw; %Web Bend-buckling must be checked for service.

%%(7b) Strength Limit State I for Negative Flexure (6.10.8)
    % 6.10.8.1.1 Discretely Braced Flanges in Compression (Bottom
    % flange is discretely braced in comp. at the neg. region)
    c(24) = Parameters_cp.Beam.fbc_neg(1,:)-Parameters_cp.Beam.Fn_neg; 
    % 6.10.8.1.3 Continuously braced flanges in Tension or Compression
    % (Top flange iin tension is continuously braced by deck at neg.
    % moment region
    c(25) = Parameters_cp.Beam.fbt_neg(1,:)-Parameters_cp.Beam.Fy;
