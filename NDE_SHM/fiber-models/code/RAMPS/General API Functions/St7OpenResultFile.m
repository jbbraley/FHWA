...
global tyBEAM rtBeamStress rtBeamForce stBeamLocal kNodeReact
%% Deck Dead Load
DeadLoadType = 1;
...
if rem(length(Parameters.Length),2) ~= 0 % for an odd number of spans
       span = ceil(length(Parameters.Length)/2);
...
for i=1:Parameters.NumGirder
    ...
    DeadLoad.StrPos(i,DeadLoadType,span, :) = BeamRes;
    DeadLoad.ForcePos(i,DeadLoadType,span,:) = BeamF;
end
end
if length(Parameters.Length)~=1
    ...
    for ii=1:floor(length(Parameters.Length)/2)
        ...
        for i=1:Parameters.NumGirder
            ...
            [iErr, ~, BeamF(:,1)] = calllib('St7API', 'St7GetBeamResultSinglePos',...
                uID, rtBeamForce, stBeamLocal, BeamNum, ResultCase, BeamPos, NumColumns, BeamF(:,1));
            HandleError(iErr);
            ...
            [iErr, ~, BeamF(:,2)] = calllib('St7API', 'St7GetBeamResultSinglePos',...
                uID, rtBeamForce, stBeamLocal, BeamNum, ResultCase, BeamPos, NumColumns, BeamF(:,2));
            HandleError(iErr);
    ...
    n=0;
    for ii=ceil(length(Parameters.Length)/2)+1:length(Parameters.Length)
        y1 = find(any(Node(ii).ID(:,:,3)),1,'first');
        y2 = find(any(Node(ii).ID(:,:,3)),1,'last');
        y = (y2-y1)/(Parameters.NumGirder-1);
        n=n+1;
...
if length(Parameters.Length)~=1
    ...
    for ii=1:floor(length(Parameters.Length)/2)
        ...
        for i=1:Parameters.NumGirder
            ...
            DeadLoad.StrNeg(i, DeadLoadType, ii, :) = BeamRes(:,2);
            DeadLoad.ForcePos(i,DeadLoadType, ii, :) = BeamF(:,2);
         end
    end
    n=0;
    for ii=ceil(length(Parameters.Length)/2)+1:length(Parameters.Length)
        ...
        for i=1:Parameters.NumGirder
            ...
            DeadLoad.StrNeg(i, DeadLoadType, ii-1, :) = BeamRes(:,2);
            DeadLoad.ForcePos(i,DeadLoadType, ii-1, :) = BeamF(:,2);
         end
    end
end
