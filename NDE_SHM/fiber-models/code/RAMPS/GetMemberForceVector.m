function [M_Max, M_Min, V_Max, V_Min, M, V] = GetMemberForceVector(Delta, loadType, Parameters, numEle)
...
for i=1:numEle % for each ele 
    for j=1:size(Delta,2) % for each load location (lane loads have a singular value
        ...
        V(i,j,:) = eleK(1,:)*squeeze(Delta(Loc:Loc+3,j,:)) - eleFEM(1,:);
        M(i,j,:) = -1*eleK(2,:)*squeeze(Delta(Loc:Loc+3,j,:)) + eleFEM(2,:);
        V(i+1,j,:) = -1*eleK(3,:)*squeeze(Delta(Loc:Loc+3,j,:)) + eleFEM(3,:);
        M(i+1,j,:) = eleK(4,:)*squeeze(Delta(Loc:Loc+3,j,:)) - eleFEM(4,:);
    end
end
