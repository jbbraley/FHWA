function LoadCase = ApplyLaneLoads(uID, Parameters, ArgIn, LoadCase, LaneBound,Node)
...
for kk = 1:size(LaneBound,2)
    for ii=1:Parameters.Spans
        ...
        for jj=1:ArgIn.NumLane
            ...
            % Create Load Case
            iErr = calllib('St7API', 'St7NewLoadCase', uID, ['Live Load', ' - Lane ', num2str(m)]);
            HandleError(iErr);
