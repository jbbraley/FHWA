function NCHRP_DeadLoadSolver_v2(uID, ModelName, ScratchPath, Parameters, Node, DLCase, FCaseNum1, FCaseNum2)

% FCaseNum1 = Freedom Case number for pinned
% FCaseNum2 = freedom case number for fixed rotation (intagral abutments)
...
HandleError(iErr);
iErr = calllib('St7API', 'St7EnableLSALoadCase', uID, LCaseNum, FCaseNum1);
HandleError(iErr);
...
switch Parameters.structureType
    case 'Steel'
        % Dead Load 1
        BmPoff{1,:} = {'Right Barrier' 'Left Barrier' 'Barrier' 'Girder Concrete'};
        BmEoff{1,:} = {'Right Barrier' 'Left Barrier' 'Barrier' 'Girder Concrete'};
        PlPoff{1,:} = {'Sidewalk'};
        ...
        BmEoff{2,:} = {'Right Barrier' 'Left Barrier' 'Barrier' 'Girder Concrete'};
        PlPoff{2,:} = {'Deck'};
        PlEoff{2,:} = {'Sidewalk'};
        % Dead Load 3 (Wearing Surface)
        if Parameters.Deck.WearingSurface ~=0
            BmPoff{3,:} = {'Ext Girder' 'Int Girder' 'Ext Girder Coverplate' 'Int Girder Coverplate' 'End Diaphragm' 'Int Diaphragm' 'Right Barrier' 'Left Barrier' 'Barrier' 'Girder Concrete'};
            BmEoff{3,:} = {};
    ...
    case 'Prestressed'
        % Dead Load 1
        BmPoff{1,:} = {'Right Barrier' 'Left Barrier' 'Barrier'};
        BmEoff{1,:} = {'Right Barrier' 'Left Barrier' 'Barrier'};
        PlPoff{1,:} = {'Sidewalk'};
        PlEoff{1,:} = {'Sidewalk' 'Deck'};
        % Dead Load 2 (Superimposed)
        BmPoff{2,:} = {'Girder Concrete' 'End Diaphragm' 'Int Diaphragm' 'Diaphragm Concrete'};
        BmEoff{2,:} = {'Right Barrier' 'Left Barrier' 'Barrier'};
        PlPoff{2,:} = {'Deck'};
        PlEoff{2,:} = {'Sidewalk'};
        % Dead Load 3 (Wearing Surface)
        if Parameters.Deck.WearingSurface ~=0
            BmPoff{3,:} = {'End Diaphragm' 'Int Diaphragm' 'Right Barrier' 'Left Barrier' 'Barrier' 'Diaphragm Concrete' 'Girder Concrete'};
            BmEoff{3,:} = {};
            PlPoff{3,:} = {'Deck' 'Sidewalk'};
            PlEoff{3,:} = {};
        end
...
for jj=1:size(BmPoff,1)
    ...
    % Alter fixity between spans for prestressed bridges
    if Parameters.Spans>1 && strcmp(Parameters.structureType, 'Prestressed')
        if jj==1
            Status = 'released';
        else
            Status = 'fixed';
        end
        [BmNums] = SetSpanContinuity( Node, Parameters, Status, 'r', BmNums, uID );
    end
    
    % Enable freedom case for integral abutments for DL2 only
    if ~isempty(FCaseNum2) && jj == 2
        
        % Disable pinned freedom case
        iErr = calllib('St7API', 'St7DisableLSALoadCase', uID, LCaseNum, FCaseNum1);
        HandleError(iErr);
        % Enable fixed rotaion freedom case
        iErr = calllib('St7API', 'St7EnableLSALoadCase', uID, LCaseNum, FCaseNum2);
        HandleError(iErr);
    ...
    %% Run Solver
    DeadLoadResultPath = [ScratchPath ModelName '_' num2str(DeadLoadType) '.lsa'];
