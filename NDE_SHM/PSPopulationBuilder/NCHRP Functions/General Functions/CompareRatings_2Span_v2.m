...
modelPath = 'L:\NCHRP Phase II\Pre-Stressed\2-Span\Suite 1\Model Files';
resultPath = 'L:\NCHRP Phase II\Pre-Stressed\2-Span\Suite 1\Extracted Result Files';
...
for ii = 1:100
    ...
    St1NegCap_Int(ii,1) = Parameters.Beam.Capacity.Mn_neg; St1NegCap_Ext(ii,1) = Parameters.Beam.Capacity.Mn_neg;
    ...
    DL_Int = DL1_M1([negLB:negUB],2:end-1)...
                + DL2_M1([negLB:negUB],2:end-1)...
                -(DL2_A([negLB:negUB],2:end-1)*y);
    % Interior LLs_b
    LL_Int = LL_M1(negLB:negUB,2:end-1)...
                - (LL_A(negLB:negUB,2:end-1)*y); 
            
    % Exterior DL1s_b + DL2s_b
    DL_Ext = DL1_M1([negLB:negUB],[1 end])...
                + DL2_M1([negLB:negUB],[1 end])...
                -(DL2_A([negLB:negUB],[1 end])*y);
    % Exterior LLs_b
    LL_Ext = LL_M1(negLB:negUB,[1 end])...
                - (LL_A(negLB:negUB,[1 end])*y);
    
    % St1_Neg FE Rating Factors
    St1Neg_FEInt(ii,1) = min(min((0.9*St1NegCap_Int(ii,1) - 1.25*abs(DL_Int))./(1.75*abs(LL_Int))));
    St1Neg_FEExt(ii,1) = min(min((0.9*St1NegCap_Ext(ii,1) - 1.25*abs(DL_Ext))./(1.75*abs(LL_Ext))));
...
plotType = 'Hist';
SaveDir = 'L:\NCHRP Phase II\Pre-Stressed\2-Span\Suite 1\RF';
